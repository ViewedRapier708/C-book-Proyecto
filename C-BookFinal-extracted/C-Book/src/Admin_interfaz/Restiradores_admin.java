...
import Clases_flujo_principal.restiradores;
...
public class Restiradores_admin {

    private BorderPane root;
    private ObservableList<String[]> data = FXCollections.observableArrayList();

    public Restiradores_admin() {
        buildUI();
        cargarDatos();
    }
    ...
    private void buildUI() {
        ...
        Label title = new Label("GESTIÓN DE RESTIRADORES");
        ...
        TableColumn<String[], String> colNum = new TableColumn<>("NÚMERO DE RESTIRADOR");
    ...
    private void cargarDatos() {
        ...
        try (Connection con = DriverManager.getConnection(Conexion_BD.url, Conexion_BD.user, Conexion_BD.pws)) {
            PreparedStatement ps = con.prepareStatement("SELECT Num_res, Estado_res FROM biblioteca.restirador");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                String estado = rs.getString("Estado_res").equals("0") ? "inactivo" : "activo";
                data.add(new String[]{rs.getString("Num_res"), estado});
            }
            rs.close(); ps.close();
    ...
    private void ejecutarOp(String op) {
        ...
        switch (op) {
            case "AGREGAR" -> {
                ...
                d.setTitle("Agregar Restiradores");
                ...
                d.showAndWait().ifPresent(input -> {
                    ...
                    try (Connection con = DriverManager.getConnection(Conexion_BD.url, Conexion_BD.user, Conexion_BD.pws)) {
                        ResultSet rs = con.prepareStatement("SELECT MAX(Num_res) FROM biblioteca.restirador").executeQuery();
                        int max = 0; if (rs.next()) max = rs.getInt(1);
                        PreparedStatement ps = con.prepareStatement("INSERT INTO biblioteca.restirador(Id_restirador,Num_res,Estado_res) VALUES(?,?,?)");
                        for (int i = 1; i <= cant; i++) {
                            int n = max + i; ps.setInt(1, n); ps.setInt(2, n); ps.setBoolean(3, true); ps.executeUpdate();
                            Ingreso_de_materiales_a_arreglos.restiradores.add(new restiradores(n, false));
                        }
                        ps.close();
                        cargarDatos();
                    } catch (Exception e) { System.out.println(e.getMessage()); }
            ...
            case "ELIMINAR" -> {
                TextInputDialog d = new TextInputDialog();
                d.setTitle("Eliminar Restirador");
                d.setHeaderText(null);
                d.setContentText("Número del restirador a eliminar:");
                d.showAndWait().ifPresent(input -> {
                    if (!input.trim().matches("^[0-9]+$")) return;
                    try (Connection con = DriverManager.getConnection(Conexion_BD.url, Conexion_BD.user, Conexion_BD.pws)) {
                        PreparedStatement ps = con.prepareStatement("DELETE FROM biblioteca.restirador WHERE Num_res = ?");
            ...
            case "MODIFICAR" -> {
                ...
                d.setHeaderText(null);
                d.setContentText("Número del restirador:");
                d.showAndWait().ifPresent(input -> {
                    if (!input.trim().matches("^[0-9]+$")) return;
                    ...
                    try (Connection con = DriverManager.getConnection(Conexion_BD.url, Conexion_BD.user, Conexion_BD.pws)) {
                        PreparedStatement ps = con.prepareStatement("UPDATE biblioteca.restirador SET Estado_res = ? WHERE Num_res = ?");
